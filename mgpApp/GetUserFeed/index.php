...
try {
    ...
    $pk = $_GET['insta_pk'];
    if ($pk){
        $user_info = $ig->people->getInfoById($pk);
        $userId = $pk;
    } 
 else {
...
//    }
    ...
    for ($i = 1; $i <= 1; $i++) {
        // echo $mxid ; 
        $response = $ig->timeline->getUserFeed($userId, $mxid);
        $myTimeline = $response;
        ...
        foreach ($myTimeline->getItems() as $item) {
            ...
            $mediaFiles[$itemId] = [
                'media' => $mediaFilesdata,
                'submedia' => $submediaFilesdata
            ];
            $mediaFilesdata = [];
            $submediaFilesdata = [];
            $subPosition=0;
    ...
    foreach ($mediaFiles as $key => $value) {
     //   var_dump($value['media'][$key]['mediaUrl']);
     //   var_dump(json_encode($value['submedia'],TRUE) );
//        foreach ($value['submedia'] as  $skey=>$svalue){
//          echo '***';var_dump(json_encode($svalue,TRUE));  
//        }
//        if ($tt++ > 3) {
//            break;
//        }

        //var_dump($value['submedia']);
//                    foreach ($value['submedia'] as  $skey=>$svalue){
//                     echo '\n';   
//                      var_dump($value[$key]['url']);
//                     echo '@@@@@@@@@@@';
//                    }
//                echo $key;
//                echo '<br>';
